...
class CreateTeacherProfile extends StatelessWidget {
  ...
  final String type;
  CreateTeacherProfile({this.teacher, this.type});
  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
          ProfileBloc(updateProfileRepository: UpdateProfileRequest()),
      child: CreateTeacherProfilePage(teacher, type),
    );
  }
}

// ignore: must_be_immutable
class CreateTeacherProfilePage extends StatelessWidget {
  final Teachers teachers;
  final String type;
  CreateTeacherProfilePage(this.teachers, this.type);
  ...
  void setCurrency(){
    getUserCurrency().then((currency) {
      _currency=currency;
      _profileBloc.add(ChangeGenderEvent(genderDropdownValue));/// This rebuilds ui for currency
    });
  }
  ...
  void populateForm(){
    if(type=='Edit'){
      ...
      emailController.text=teachers.email;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title:
          Text("$type Teacher", style: TextStyle(color: MyColors.primaryColor)),
          elevation: 0.0,
    ...
    return Form(
      ...
      child: Column(
        children: <Widget>[
            ...
            child: Card(
              ...
              child: Padding(
                ...
                child: InkWell(
                  ...
                  onTap: ()async{
                    LaunchRequest().launchURL('${MyStrings.frontEndDomain}/authenticator?api_token=${await getApiToken()}&page_name=create_teacher_csv');
                  },
                ),
              ),
            ),
          ...
          (type=='Add')?BlocListener<ProfileBloc, ProfileStates>(listener:
        ...
        onPressed: () {
          if(validateSpinner()){
            if (_formKey.currentState.validate()) {
              switch (type){
                case 'Add':{
                  _profileBloc.add(CreateTeacherProfileEvent(roleController.text, titleController.text, fNameController.text, lNameController.text, oNameController.text, userNameController.text, passwordController.text, genderDropdownValue, phoneController.text, emailController.text, addressController.text, cityController.text, stateController.text, _country, salaryController.text.replaceAll(',', ''), formatDate.format(selectedDate), bankSpinnerIds[bankSpinnerItems.indexOf(bankDropdownValue)], bankDropdownValue, bankAccountNumberController.text, ''));
                  break;
                }
                case 'Edit':{
                  _profileBloc.add(UpdateTeacherProfileEvent(teachers.id, titleController.text, fNameController.text, lNameController.text, oNameController.text, genderDropdownValue, phoneController.text, addressController.text, cityController.text, stateController.text, _country, formatDate.format(selectedDate), roleController.text, salaryController.text.replaceAll(',', ''), teachers.preferredCurrencyId, teachers.bankCode, teachers.bankName, teachers.bankAccountNumber));
                  break;
                }
              }
