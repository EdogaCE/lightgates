...
import 'package:school_pal/ui/bill_payment.dart';
import 'package:school_pal/ui/general.dart';
import 'package:school_pal/ui/web_view_loader.dart';
import 'package:school_pal/utils/system.dart';
import 'package:flutter_speed_dial/flutter_speed_dial.dart';

class ViewBillPayment extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => WalletBloc(viewWalletRepository: ViewWalletRequest()),
      child: ViewBillPaymentPage(),
    );
  }
}

// ignore: must_be_immutable
class ViewBillPaymentPage extends StatelessWidget with WidgetsBindingObserver {
  WalletBloc _walletBloc;
  List<School> _transactionHistory=List();
  String _currency;
                ...
                actions: <Widget>[
                  IconButton(
                      ...
                      onPressed: () {
                        showSearch(
                            context: context,
                            delegate: CustomSearchDelegate(_transactionHistory));
                      })
                ],
                pinned: true,
      ...
      floatingActionButton: SpeedDial(
        ...
        children: [
          SpeedDialChild(
            ...
            onTap: (){
              _navigateToBillPaymentScreen(context, false).then((result){
                if(result!=null){
                  Scaffold.of(context)
          ...
          SpeedDialChild(
              ...
              labelStyle: TextStyle(fontSize: 18.0),
              onTap: () {
                _navigateToBillPaymentScreen(context, true).then((result){
                  if(result!=null){
                    _viewFoundHistory();
                  }
                });
    ...
    return BlocListener<WalletBloc, WalletStates>(
      listener: (context, state) {
        ...
        if (state is NetworkErr) {
          ...
          _transactionHistory = state.school.where((element) => element.walletRecord.billPayment).toList();
      ...
      child: BlocBuilder<WalletBloc, WalletStates>(
        builder: (context, state) {
          ...
          if (state is WalletInitial) {
            ...
            return _transactionHistory.isNotEmpty
            ? _buildLoadedScreen(context, _transactionHistory)
                : buildLoadingScreen();
          } else if (state is TransactionHistoriesLoaded) {
            return _transactionHistory.isNotEmpty
                ? _buildLoadedScreen(context, _transactionHistory)
 ...
 Future<String> _navigateToBillPaymentScreen(BuildContext context, bool isAirtime) async {
      final result = await Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => BillPayment(isAirtime: isAirtime)),
      );
      return result;
  }
